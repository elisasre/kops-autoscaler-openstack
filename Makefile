...
build-linux-amd64:
	...
	go version
	GOOS=linux GOARCH=amd64 CGO_ENABLED=0 GO111MODULE=on go build -mod vendor -v -o bin/linux/$(OPERATOR_NAME) ./cmd

build:
	rm -f bin/$(OPERATOR_NAME)
	GO111MODULE=on go build -mod vendor -v -o bin/$(OPERATOR_NAME) ./cmd
